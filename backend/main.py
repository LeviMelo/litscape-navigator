...
class StatusResponse(BaseModel):
    job_id: str
    status: str  # PENDING, RUNNING, STARTED, COMPLETED, FAILED
...
async def start_search_job(search_request: SearchRequest):
    ...
    log.info(f"Received job start request {job_id} with query: '{search_request.query}'")
    if not database.add_job(job_id, search_request.query):
         log.error(f"Failed to add job {job_id} to database.")
         raise HTTPException(status_code=500, detail="Database error.")
    try:
        run_full_pipeline.delay(job_id=job_id, query=search_request.query, max_results=search_request.max_results)
        log.info(f"Task for job {job_id} dispatched to Celery.")
    except Exception as e:
         log.error(f"Failed to dispatch task for job {job_id}: {e}", exc_info=True)
         database.update_job_status(job_id, 'FAILED', error_message=f"Queue dispatch failure: {e}")
         raise HTTPException(status_code=500, detail=f"Queue task failure: {e}")
...
async def get_search_results(job_id: str):
    log.info(f"Retrieving results for job {job_id}")
    status_info = database.get_job_status(job_id)
    if status_info is None:
    ...
    if not result_paths:
         raise HTTPException(status_code=500, detail="Result paths missing.")
    data_path = result_paths.get('results_data_path')
    graph_path = result_paths.get('results_graph_path')
    if not data_path:
         raise HTTPException(status_code=500, detail="Data path missing.")
    log.info(f"Loading merged data from {data_path} for job {job_id}")
    ...
    try:
         with open(data_path, 'r', encoding='utf-8') as f:
             merged_data_content = json.load(f)
         log.info(f"Merged data loaded for job {job_id}")
         return BasicResultsResponse(merged_data=merged_data_content, graph_info=GraphInfo(**graph_info_data))
    except Exception as e:
         log.error(f"Failed to load/process results for job {job_id}: {e}", exc_info=True)
